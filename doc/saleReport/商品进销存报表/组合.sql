select
   ...
   IFNULL(DATEDIFF(curdate(),DATE_ADD(curdate(), interval -day(curdate()) + 1 day)) * 0.5 *(sum(initial_num)+(sum(initial_num) + sum(current_in_num) - sum(current_out_num)))/sum(current_normal_out_num),0) inventory_turnover_days,
   IFNULL(360/(DATEDIFF(curdate(),DATE_ADD(curdate(), interval -day(curdate()) + 1 day)) * 0.5 *(sum(initial_num)+(sum(initial_num) + sum(current_in_num) - sum(current_out_num)))/sum(current_normal_out_num)),0) inventory_turnover
from (
         ...
         from (SELECT projectId,
               ...
               FROM (
                        SELECT tbis.project_id projectId,
                               ...
                               tbisd.receive_num,
                               (tbisd.receive_num * tbisd.cost_price) receive_amount,
                               tbisd.receive_price,
                               tbisd.cost_price,
               ...
               FROM (
                        SELECT
                               ...
                               (tbosd.cost_amount/tbosd.send_num) cost_price,
         ...
         from (select projectId,
               ...
               from (select tbis.project_id projectId,
                     ...
                     where 1 = 1
                       ...
                       AND tbis.receive_date >= '2019-12-01 00:00:00'
                       AND tbis.receive_date <= '2019-12-26 00:00:00'
                       AND tbis.status = 2
                       and tbp.id = 13
                       and tbp.business_unit_id = 7
         ...
         from (select projectId,
               ...
               from (SELECT tbos.project_id projectId,
                     ...
                     where 1 = 1
                       ...
                       AND tbos.send_date >= '2019-12-01 00:00:00'
                       AND tbos.send_date <= '2019-12-26 00:00:00'
                       AND tbos.status = 5
                       and tbp.id = 13
                       and tbp.business_unit_id = 7
                    ) temp1 group by projectId, busiUnitId, goodsId
               ...
               from (SELECT tbos.project_id projectId,
                     ...
                     where 1 = 1
                       ...
                       AND bup.state = 1
                       AND (tbs.subject_type & 1 = 1)
                       and (tbos.bill_type = 2 or tbos.bill_type = 3 or tbos.bill_type = 4 or
                            tbos.bill_type = 6 or
     ...
     ) temp4 group by projectId, busiUnitId, goodsId
