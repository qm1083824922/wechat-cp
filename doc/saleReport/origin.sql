select projectId,
       ...
       IFNULL(DATEDIFF(DATE_FORMAT('', "%Y-%m-%d"), DATE_FORMAT(?, "%Y-%m-%d")) * 0.5 *
       ...
       IFNULL(360 / (DATEDIFF(DATE_FORMAT(?, "%Y-%m-%d"), DATE_FORMAT('', "%Y-%m-%d")) * 0.5 *
                     (sum(initial_num) + (sum(initial_num) + sum(current_in_num) - sum(current_out_num))) /
      ...
      from (SELECT projectId,
            ...
            FROM (SELECT tbis.project_id                           projectId,
                  ...
                  WHERE 1 = 1
                    ...
                    and DATE_FORMAT(tbis.receive_date, "%Y-%m-%d") <= ?
                    AND tbp.id = ?
                    -- AND tbg.number = ?
                    -- AND tbg.bar_code = ?
                ) temp
            ...
            FROM (SELECT tbos.project_id      projectId,
                  ...
                  WHERE 1 = 1
                    ...
                    AND tbos.STATUS = 5
                    AND (tbs.subject_type & 1 = 1)
                    and DATE_FORMAT(tbos.send_date, "%Y-%m-%d") <= ?
                    AND tbp.id = ?
                    -- AND tbg.number = ?
                    -- AND tbg.bar_code = ?
                ) temp2
