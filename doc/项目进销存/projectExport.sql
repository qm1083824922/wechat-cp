select
    ...
    businessUnitName,
    initial_num,
    round(initial_amount,2) initial_amount,
    initial_in_num,
    round(initial_in_amount,2) initial_in_amount,
    initial_out_num,
    round(initial_out_amount,2) initial_out_amount,
    current_in_num,
    round(current_in_amount,2) current_in_amount,
    current_normal_in_num,
    round(current_normal_in_amount,2) current_normal_in_amount,
    current_other_in_num,
    round(current_other_in_amount,2) current_other_in_amount,
    current_out_num,
    round(current_out_amount,2) current_out_amount,
    current_normal_out_num,
    round(current_normal_out_amount,2) current_normal_out_amount,
    current_other_out_num,
    round(current_other_out_amount,2) current_other_out_amount,
    end_num,
    round(end_amount,2)end_amount,
     ...
     from
         (SELECT
          ...
          WHERE
            ...
            AND tbis.receive_date <= '2019-12-01'
            AND tbis.STATUS = 2
          ...
          WHERE
            ...
            AND ( tbs.subject_type & 1 = 1 )
            AND tbis.receive_date >= '2019-12-01'
            AND tbis.receive_date <= '2019-12-31'
          ...
          WHERE
            ...
            AND tbos.STATUS = 5
          group by tbp.id,tbp.business_unit_id
         )temp group by project_id,business_unit_id
    ) temp2 group by project_id,business_unit_id
    ) temp3 group by project_id,business_unit_id
