SELECT v_table.project_id,
       ...
       (SUM(v_table.store_num) - SUM(v_table.lock_num) -
        SUM(v_table.sale_lock_num))                                                                       sum_not_lock_num,
       IF(v_table.in_store_time > 0 && v_table.in_store_time < 20, sum(v_table.store_num),
          0)                                                                                              in_library_count1,
       IF(v_table.in_store_time > 0 && v_table.in_store_time < 20, sum(v_table.sum_store_amount),
          0)                                                                                              in_library_amount1,
       IF(v_table.in_store_time >= 20 && v_table.in_store_time < 60, sum(v_table.store_num),
          0)                                                                                              in_library_count2,
       IF(v_table.in_store_time >= 20 && v_table.in_store_time < 60, sum(v_table.sum_store_amount),
          0)                                                                                              in_library_amount2,
...
from (select s.id,
      ...
      where 1 = 1
        ...
        and bup.state = 1
        and (s.store_num > 0)
    ) as v_table group by v_table.project_id,v_table.business_unit_id
